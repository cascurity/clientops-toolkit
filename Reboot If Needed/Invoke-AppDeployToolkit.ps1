...
## Run with powershell.exe -File "<path-to>\Invoke-AppDeployToolkit.ps1" -WebhookId <webhook-id> -WebhookToken <webhook-token>
...
function Install-ADTDeployment
{
	...
	if ((Get-ADTPendingReboot).IsSystemRebootPending) {
		Show-ADTInstallationPrompt -Icon Information -ButtonRightText 'OK' -Message 'Your system detected that a reboot is needed'
		Show-ADTInstallationRestartPrompt -CountdownSeconds 300 -NotTopMost
		$message = @{
			content = 'Reboot initiated'
		} | ConvertTo-Json
		try
		{
