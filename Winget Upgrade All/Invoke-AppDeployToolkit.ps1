...
## powershell.exe -File "<path-to>\Invoke-AppDeployToolkit.ps1" -WebhookId "<webhook-id>" -WebhookToken "<webhook-token>"
...
$adtSession = @{
    ...
    AppScriptDate = '2025-07-15'
    AppScriptAuthor = 'cascuity.ch'
...
function Install-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt -Title "Installing Upgrades" 
	...
	try
	{
		Start-ADTProcess -UseShellExecute -FilePath 'powershell.exe' -ArgumentList '-Command "winget upgrade --all --nowarn --accept-source-agreements --accept-package-agreements"'
	} 
	catch
	{
		$errorOnInstall = $true
		Write-ADTLogEntry -Message "Error Installing Patches"
	}
	...
	if(!$errorOnInstall)
	{
		$msg = 'The Upgrades installed successfully.'
	}
	else
	{
		$msg = 'An ERROR encountered while installing. Please try again or contact your Admin.'
	}
	...
	{
		Write-ADTLogEntry -Message "Error Uploading to Discord"
	}
	
	Show-ADTInstallationPrompt -Message $msg -ButtonRightText 'OK' -Icon Information -NoWait
